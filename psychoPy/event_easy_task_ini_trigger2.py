...
expName = u'checkerboard_event2_easy'  # from the Builder filename that created this script
...
trialClock_all = core.Clock()
LCM=1/time_period/4*2     # LCM*4*time_period=min interval=2 sec
press1_count=0
trigger_count=0
time_stamp=0
press_any=0
...
tmp_event2.extend([5]*(int( 60)))# condition5: R+L high
tmp_event2.extend([6]*(int( 0)))# condition6: R+L low
tmp_event2.extend([7]*(int( 0)))# condition7: R+Lsmall high
tmp_event2.extend([0]*(int( total_scan/LCM/4)-60))# gitter
...
#print tmp_event
i=0
tmp_event=[tmp_event2[i]]
for ii in range(int(LCM)-1):
    tmp_event.extend([0])

while i<len(tmp_event2)-1:
    i=i+1
...
for i in range(len(param_event_series)):
    param.extend( [param_event_series[i]] * 4)#4=> 125ms*4=500ms(event stimlus : flip 4 times)


for i in range(len(param_event_series)):
    contrast_flag.extend( [contrast_flag_series[i]] * 4)
...
while continueRoutine:
    ...
    if keybrd_input.status == STARTED and trigger_count==0:
        # listen key input
    ...
    if t >= 0.0 and trigger_count>0 and t > trigger_time and t <= trigger_time+total_time and End_instruction.status == NOT_STARTED :
        ...
        if "5" in theseKeys or "5,2" in theseKeys or "2,5" in theseKeys:
            trigger_count=trigger_count+1
            reset_time=(trigger_count-1)*TR+trigger_time
            trialClock.reset(-1*reset_time)
    ...
    if trigger_count>0 and t >=trigger_time+(time_frame_count+1)*time_period and t <= trigger_time+total_time and trigger_count/5>=time_frame_count/8:
        time_frame_count=time_frame_count+1

    if trigger_count%5==1 and trigger_count>0 and t >=trigger_time+(time_frame_count+1)*time_period and t <= trigger_time+total_time:
        time_frame_count=int((trigger_count-1)/5*8)+1
    ...
    if t >= 0.0 and trigger_count>0 and t > trigger_time and t < trigger_time+total_time-TR and End_instruction.status == NOT_STARTED :
